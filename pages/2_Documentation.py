...
st.write('''The urea clearance shown in the visualizer app is computed through a support vector regression model 
         trained on synthetic data generated by a Computationl Fluid Dynamics (CFD) model of the dialyzer. The CFD model 
         computes the clearance by solving fluid dynamics and scalar transport equations, obtaining pressure, concentration and flow rate profiles. 
...
st.write("For more information, check the published [article](%s) where the same methodology was applied to a create a surrogate of a 1D model. Article regarding the development of the CFD surrogate model coming soon." % url)
